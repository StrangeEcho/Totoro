...
class Searches(commands.Cog):
    ...
    async def cog_load(self):
        ...
        self.logger.info("Finished establishing async_cse Client. Ready to interact with api...")
    ...
    async def search(self, ctx: commands.Context, *, query: str):
        ...
        try:
            results: list[async_cse.Result] = await self.async_cse.search(
                query
            )
            await ctx.send(
                embed=discord.Embed(
                    title=query,
                    description="\n\n".join([f"[{search.title}]({search.url})\n{search.description}" for search in results[:5]]),
